...
func _set_config(config_json : Dictionary) -> void:
	_config = config_json
	if _signup_request_url.find("%s") != -1:
		_signup_request_url %= _config.apiKey
	if _signin_request_url.find("%s") != -1:
		_signin_request_url %= _config.apiKey
	if _signin_custom_token_url.find("%s") != -1:
		_signin_custom_token_url %= _config.apiKey
	if _signin_with_oauth_request_url.find("%s") != -1:
		_signin_with_oauth_request_url %= _config.apiKey
	if _userdata_request_url.find("%s") != -1:
		_userdata_request_url %= _config.apiKey
	if _refresh_request_url.find("%s") != -1:
		_refresh_request_url %= _config.apiKey
	if _oobcode_request_url.find("%s") != -1:
		_oobcode_request_url %= _config.apiKey
	if _delete_account_request_url.find("%s") != -1:
		_delete_account_request_url %= _config.apiKey
	if _update_account_request_url.find("%s") != -1:
